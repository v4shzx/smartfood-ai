      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex items-center gap-2 mb-2 text-emerald-600 dark:text-emerald-400 font-black text-[10px] uppercase tracking-[0.3em]">
            <LayoutDashboard className="w-3.5 h-3.5" /> {t.dashboard.charts}
          </div>
          <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white leading-none">{t.dashboard.charts}</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-3 font-medium text-lg">Analisis visual de ingresos, tickets y productos estrella.</p>
        </motion.div>
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <ReLineChart data={chartsSales}>
                ...
                <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 11, fontWeight: 700 }} stroke="rgba(148,163,184,0.5)" dy={10} />
                <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fontSize: 11, fontWeight: 700 }} stroke="#10b981" tickFormatter={(v) => `$${v >= 1000 ? (v / 1000).toFixed(1) + "k" : v}`} />
                <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 11, fontWeight: 700 }} stroke="#3b82f6" />
                <Tooltip
                ...
                />
                <Legend iconType="circle" wrapperStyle={{ paddingTop: 30, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em" }} />
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartsTopProducts} layout="vertical" margin={{ left: -20, right: 20 }}>
                ...
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 800, width: 80 }} stroke="rgba(148,163,184,0.8)" />
